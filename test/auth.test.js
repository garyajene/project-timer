...
test('registration is available without feature flags', async (t) => {
  const { url } = await fixture(t);
  assert.equal((await post(url, '/api/auth/register', { email: 'a@example.com', password })).status, 201);
});
...
test('first account gets a verified legacy copy while app_state stays untouched and later accounts start empty', async (t) => {
  const { store, url } = await fixture(t);
  await store.save(richState);
  const legacyBefore = await store.query('SELECT state_json FROM app_state WHERE id=1;');
  const ownerCookie = cookieOf(await post(url, '/api/auth/register', { email: 'first@example.com', password }));
  ...
  assert.equal((await post(url, '/api/auth/register', { email: 'FIRST@example.com', password })).status, 409);
