...
function saveState() {
  ...
  pendingSave = pendingSave.catch(() => {}).then(async () => {
    if (!authenticatedUser || saveGeneration !== authGeneration) return false;
    ...
    stateRevision = (await response.json()).revision;
    return true;
  }).catch((error) => {
