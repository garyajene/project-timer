...
export class StateStore {
  ...
  async register(emailValue, password) {
    ...
    let isOwner = false;
    try {
      const result = await this.query(`BEGIN IMMEDIATE;
        ...
        INSERT INTO user_states (user_id,state_json,revision,updated_at)
          SELECT ${sqlString(id)}, CASE WHEN (SELECT COUNT(*) FROM users) = 1 THEN state_json ELSE ${sqlString(JSON.stringify(EMPTY_STATE))} END, 0, CURRENT_TIMESTAMP FROM app_state WHERE id = 1;
        SELECT CASE WHEN (SELECT COUNT(*) FROM users) = 1 THEN 1 ELSE 0 END;
        COMMIT;`);
      isOwner = result.split('\n').at(-1) === '1';
